...
- Can use training set mean and covariance in place of $\boldsymbol{\mu}_i$ and $\Sigma$
...
## Cross-validation on training set data
...
## For fun: comparison to logistic regression

```{r logistic-comparison, echo=FALSE, warning=FALSE, results="asis", message=FALSE}
eq_dat <- data.frame(earthquake=c(rep(1,8),rep(0,8)),
                     P_mag=c(eq.P,ex.P),
                     S_mag=c(eq.S,ex.S))
logistic_fit <- glm(as.factor(earthquake)~.,data=eq_dat,
                    family="binomial")
print(xtable(summary(logistic_fit)), comment=FALSE)
nz_dat <- data.frame(earthquake=0.5,
                     P_mag=NZ.P,
                     S_mag=NZ.S)
NZ_pred <- predict(logistic_fit,nz_dat, type = "response", se.fit=T)

paste("The prediction for NZ is", signif(NZ_pred$fit,3), ", with standard error", signif(NZ_pred$se.fit,3))
```
...
$$
log(f_i(\mathbf{X}))=-\sum_{0<\omega_k<1/2}[log|S_i|+\mathbf{X}^*S_i^{-1}\mathbf{X}-2\mathbf{M}^*_iS_i^{-1}\mathbf{X}+\mathbf{M}^*_iS_i^{-1}\mathbf{M}_i]
$$

- If $\mathbf{M}_1=\mathbf{M}_2=0$, then:
$$
log(\frac{f_1(\mathbf{X})}{f_2(\mathbf{X})})=\sum_{0<\omega_k<1/2}[-log\frac{|S_1|}{|S_2|}-tr\{\mathbf{X}^*(S_1^{-1}-S_2^{-1})\mathbf{X}\}]
$$


## Measures of disparity: Kullback-Leibler

- Not really sure why we can't compute this quantity, instead we use two measures of disparity that are related to this quantity
- Kullback-Leibler (K-L) discrimination information:
$$
I(\theta_1,\theta_2)=\frac{1}{n}E_1log(\frac{f_1(y;\theta)}{f_2(y;\theta)})
$$
    - If $I(\theta_1,\theta_2)=0$, the two probability distributions are the same; positive if the $f_1$ has greater probability
- Using this, they transform our problem into:
$$
\frac{1}{n}E_1log(\frac{f_1(\mathbf{X})}{f_2(\mathbf{X})}) = \frac{1}{n}\sum_{0<\omega_k<1/2}[tr\{\frac{|S_1|}{|S_2|}\}-log(\frac{|S_1|}{|S_2|})-p]
$$
    
## Measures of disparity: Chernoff

- Chernoff (CH) information measure:
$$
B_\alpha(y=1,y=2)=-logE_2\{(\frac{f_2(\mathbf{x})}{f_1(\mathbf{x})})^\alpha\}
$$
    - which has symmetric divergence when $\alpha=0.5$
- They transform this into the following for our scenario:
$$
B_\alpha=\frac{1}{2n}\sum_{0<\omega_k<1/2}[log(\frac{|\alpha S_1 + (1-\alpha)S_2|}{|S_2|})- \alpha log(\frac{|S_1|}{|S_2|})]
$$
    - where the probability that $y=1$ increases as $B_\alpha$ decreases
